...
ASSEMBLER_NAME = 'MiniasmRaconAvgQv'
...
def run(datasets, output_path, approx_genome_len=0, move_exiting_out_path=True):
    ...
    if machine_name == 'pacbio' or machine_name == 'nanopore':
        ...
        # execute_command(command, fp_log, dry_run=DRY_RUN);
        current_memtime_id += 1;
        command = '%s %s -M 5 -X -4 -G -8 -E -6 --bq 21 -t %d %s %s %s %s' % (measure_command('%s-%s.memtime' % (memtime_files_prefix, current_memtime_id)), RACON_BIN, num_threads, reads_file, mapping_iter1_paf, assembly_iter0_fasta, assembly_iter1_fasta);
        execute_command(command, fp_log, dry_run=DRY_RUN);
        ...
        # execute_command(command, fp_log, dry_run=DRY_RUN);
        current_memtime_id += 1;
        command = '%s %s -M 5 -X -4 -G -8 -E -6 --bq 21 -t %d %s %s %s %s' % (measure_command('%s-%s.memtime' % (memtime_files_prefix, current_memtime_id)), RACON_BIN, num_threads, reads_file, mapping_iter2_paf, assembly_iter1_fasta, assembly_iter2_fasta);
        execute_command(command, fp_log, dry_run=DRY_RUN);
