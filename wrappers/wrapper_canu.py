...
ASSEMBLER_BIN = os.path.join(ASSEMBLER_PATH, 'Linux-amd64/bin/canu')
ASSEMBLER_NAME = 'Canu'
...
def run(datasets, output_path, approx_genome_len=0, move_exiting_out_path=True):
    ...
    dataset_ext = '';
    for dataset in datasets:
        curr_ext = os.path.splitext(dataset.reads_path)[-1];
        if (dataset_ext == ''): dataset_ext = curr_ext;
        if (curr_ext != dataset_ext):
            log('ERROR: Not all reads files are in the same format! All must be either in FASTA or FASTQ! Exiting.\n', fp_log);
            exit(1);

    reads_file = '%s/all_reads%s' % (output_path, dataset_ext);
    for dataset in datasets:
        ...
        # else:
        log('Aggregating %s file "%s" to "%s".\n' % (dataset.reads_path, dataset_ext.upper(), reads_file), fp_log);
...
# # On worker
# cd; cd benchmark/results/canu
# /home/kresimir/src/cgmemtime/cgmemtime -o ./dataset5.memtime /home/kresimir/src/canu/Linux-amd64/bin/canu
# -d /home/kresimir/benchmark/results/canu/dataset5
# -p canud5 genomeSize=4.7m
# -nanopore-raw /home/kresimir/benchmark/datasets/dataset5_50x2d/reads.fastq

        command = '%s %s -d %s -p %s genomeSize=%s -pacbio-raw %s' % (measure_command(memtime_path), ASSEMBLER_BIN, output_path, 'canu', approx_genome_len, reads_file);
        execute_command(command, fp_log, dry_run=DRY_RUN);

    elif machine_name == 'nanopore':

        command = '%s %s -d %s -p %s genomeSize=%s -nanopore-raw %s' % (measure_command(memtime_path), ASSEMBLER_BIN, output_path, 'canu', approx_genome_len, reads_file);
        execute_command(command, fp_log, dry_run=DRY_RUN);
...
def download_and_install():
    if os.path.exists(ASSEMBLER_BIN):
        ...
        if (os.path.exists(ASSEMBLER_PATH)):
            log('Folder on path %s already exists. Moving folder to: %s.bak.' % (ASSEMBLER_PATH, ASSEMBLER_PATH), None);
#            os.rename(ASSEMBLER_PATH, '%s.bak' % (ASSEMBLER_PATH));
            shutil.rmtree(ASSEMBLER_PATH);
...
if __name__ == "__main__":
    ...
    if (sys.argv[1] == 'install'):
        ...
        approx_genome_len = sys.argv[3]; # 0 if (sys.argv[3] == '-') else int(sys.argv[3]);
        datasets = [];
