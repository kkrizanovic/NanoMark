...
ASSEMBLER_PATH = os.path.join(ASSEMBLERS_PATH_ROOT_ABS, 'miniasmracon-grpaper')
ASSEMBLER_BIN = os.path.join(ASSEMBLER_PATH,'miniasm/miniasm')
ASSEMBLER_NAME = 'MiniasmRacon-grpaper'
...
def run(datasets, output_path, approx_genome_len=0, move_exiting_out_path=True, run_type="run"):
    ...
    if machine_name == 'pacbio' or machine_name == 'nanopore':
        ...
        elif (run_type == "runqv20"):
        	command = '%s %s -M 5 -X -4 -G -8 -E -6 --bq 20 -t %d %s %s %s %s' % (measure_command('%s-%s.memtime' % (memtime_files_prefix, current_memtime_id)), RACON_BIN, num_threads, reads_file, mapping_iter1_paf, assembly_iter0_fasta, assembly_iter1_fasta);
        elif (run_type == "ovlwin"):
        	command = '%s %s -M 5 -X -4 -G -8 -E -6 --bq 10 --ovl-margin 0.1 -t %d %s %s %s %s' % (measure_command('%s-%s.memtime' % (memtime_files_prefix, current_memtime_id)), RACON_BIN, num_threads, reads_file, mapping_iter1_paf, assembly_iter0_fasta, assembly_iter1_fasta);
        elif (run_type == "ovlwinqv20"):
        	command = '%s %s -M 5 -X -4 -G -8 -E -6 --bq 20 --ovl-margin 0.1 -t %d %s %s %s %s' % (measure_command('%s-%s.memtime' % (memtime_files_prefix, current_memtime_id)), RACON_BIN, num_threads, reads_file, mapping_iter1_paf, assembly_iter0_fasta, assembly_iter1_fasta);
        execute_command(command, fp_log, dry_run=DRY_RUN);
        ...
        # execute_command(command, fp_log, dry_run=DRY_RUN);
        current_memtime_id += 1;
#        command = '%s %s -M 5 -X -4 -G -8 -E -6 --bq 10 --align -t %d %s %s %s %s' % (measure_command('%s-%s.memtime' % (memtime_files_prefix, current_memtime_id)), RACON_BIN, num_threads, reads_file, mapping_iter2_paf, assembly_iter1_fasta, assembly_iter2_fasta);
        if (run_type == "run"):
        	...
        	command = '%s %s -M 5 -X -4 -G -8 -E -6 --bq 20 -t %d %s %s %s %s' % (measure_command('%s-%s.memtime' % (memtime_files_prefix, current_memtime_id)), RACON_BIN, num_threads, reads_file, mapping_iter2_paf, assembly_iter1_fasta, assembly_iter2_fasta);
        elif (run_type == "ovlwin"):
        	command = '%s %s -M 5 -X -4 -G -8 -E -6 --bq 10 --ovl-margin 0.1 -t %d %s %s %s %s' % (measure_command('%s-%s.memtime' % (memtime_files_prefix, current_memtime_id)), RACON_BIN, num_threads, reads_file, mapping_iter2_paf, assembly_iter1_fasta, assembly_iter2_fasta);
        elif (run_type == "ovlwinqv20"):
        	command = '%s %s -M 5 -X -4 -G -8 -E -6 --bq 20 --ovl-margin 0.1 -t %d %s %s %s %s' % (measure_command('%s-%s.memtime' % (memtime_files_prefix, current_memtime_id)), RACON_BIN, num_threads, reads_file, mapping_iter2_paf, assembly_iter1_fasta, assembly_iter2_fasta);
        execute_command(command, fp_log, dry_run=DRY_RUN);
...
def download_and_install():
    if os.path.exists(ASSEMBLER_BIN):
        ...
        command = 'cd %s; git clone https://github.com/isovic/racon.git && (cd racon && git checkout grpaper && make modules && make tools && make)' % (ASSEMBLER_PATH)
        execute_command(command, None, dry_run=DRY_RUN);
...
def verbose_usage_and_exit():
    ...
    sys.stderr.write('\n')
    sys.stderr.write('\t- mode - either "run" or "install". If "install" other parameters can be omitted. "run" runs normal assembly and Racon, "qv20" runs the same with "--bq 20" threshold, and "ovlwin" runs with overlapping windows.\n')
...
if __name__ == "__main__":
    ...
    elif (sys.argv[1] in ['run', 'runqv20', 'ovlwin', 'ovlwinqv20']):
        if (len(sys.argv) < 5):
            verbose_usage_and_exit()
