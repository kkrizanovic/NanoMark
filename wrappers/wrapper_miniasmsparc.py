...
ASSEMBLER_PATH = os.path.join(ASSEMBLERS_PATH_ROOT_ABS, 'miniasmsparc')
ASSEMBLER_BIN = os.path.join(ASSEMBLER_PATH,'miniasm/miniasm')
ASSEMBLER_NAME = 'MiniasmSparc'
SPARC_BIN = os.path.join(ASSEMBLER_PATH, 'Sparc/Sparc')
BLASR_BIN = os.path.join(ASSEMBLER_PATH, 'blasrbinary/blasr')
...
def download_and_install():
    if os.path.exists(ASSEMBLER_BIN):
        ...
        command = 'cd %s; git clone https://github.com/ylipacbio/blasrbinary.git && chmod a+x %s' % (ASSEMBLER_PATH, BLASR_BIN)
        execute_command(command, None, dry_run=DRY_RUN);

        command = 'cd %s; git clone https://github.com/yechengxi/Sparc.git && cd Sparc && (g++ -O3 -o Sparc *.cpp)' % (ASSEMBLER_PATH)
        execute_command(command, None, dry_run=DRY_RUN);
